...
router.get("/operations", requireRole("SUPER_ADMIN"), async (_req, res) => {
  try {
    const [summary, sla, contractors, recent] = await Promise.all([
      pool.query(`SELECT COUNT(*)::int AS total_tickets, COUNT(*) FILTER (WHERE status IN ('OPEN','IN_PROGRESS','ON_HOLD'))::int AS active_tickets, COUNT(*) FILTER (WHERE status = 'COMPLETED')::int AS completed_tickets, COUNT(*) FILTER (WHERE status = 'FAILED')::int AS failed_tickets, ROUND((AVG(EXTRACT(EPOCH FROM (completed_at - created_at)) / 3600.0) FILTER (WHERE completed_at IS NOT NULL))::numeric, 1) AS avg_resolution_hours FROM tickets`),
      pool.query(`SELECT COUNT(*) FILTER (WHERE sla_status IN ('BREACHED','FAILED'))::int AS breached_tickets, COUNT(*) FILTER (WHERE sla_status = 'COMPLETED')::int AS completed_with_sla, COUNT(*) FILTER (WHERE sla_status IN ('ACTIVE','PAUSED'))::int AS monitored_tickets FROM ticket_sla_tracking`),
