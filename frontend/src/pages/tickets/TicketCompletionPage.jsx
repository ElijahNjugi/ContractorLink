...
export default function TicketCompletionPage() {
  ...
  const allowed = String(user?.organization_id) === String(ticket.assigned_organization_id) && String(user?.role_code).toUpperCase() === "ORG_ADMIN" && !["COMPLETED", "CANCELLED", "ON_HOLD"].includes(ticket.status);
